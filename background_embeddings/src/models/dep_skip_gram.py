...
with open('files_sg_300.txt', 'r') as f:
...
for epoch in range(epochs):
    ...
    for parsed_file in parsed_files:
        ...
        if files > max_files:
            break
        with open('files_sg_300.txt', 'w') as f:
            f.write(str(files))
            f.flush()
        for line in reader:
            words = line.split(" ")
                ...
                if words[2] != "ROOT\n":  # don't include dependencies between the same token
                    if words[0] in vocab and words[1] in vocab:
                        ...
                        if train_examples % 10000 == 0:
                            with open('losses_sg.txt', 'a') as f:
                                f.write("{}\n".format(loss_val / train_examples))
                        """
        reader.close()    
        if files % 10 == 0:
            m.save("dep_sg_model_300_complete")
